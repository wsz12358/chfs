...
namespace mapReduce {
	...
	TEST_F(MapReduceTest, DistributedMapReduce) {
		...
		std::sort(kvs.begin(), kvs.end());
/*        std::cout << "size:" << kvs.size() << "and" << correctKvs.size();
        int size_ = kvs.size() > correctKvs.size() ? correctKvs.size() : kvs.size();
        for (int i = 0; i < size_; i++)
        {
            if (kvs[i].first != correctKvs[i].first || kvs[i].second != correctKvs[i].second)
            {
                std::cout << "at " << i << "mismatches: kvs " << kvs[i].first << " " << kvs[i].second << " correct " << correctKvs[i].first << " " << correctKvs[i].second << std::endl;
            }
        }*/
		EXPECT_EQ(kvs, correctKvs) << "incorrect output";
