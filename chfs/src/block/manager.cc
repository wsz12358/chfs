...
auto BlockManager::write_block(block_id_t block_id, const u8 *data)
  ...
  if (this->is_log_enabled)
    ops.push_back(std::make_shared<BlockOperation>(block_id, std::vector<chfs::u8>(data, data + block_sz)));
  u8 * ptr = this->block_data;
  ptr += block_id * block_sz;
...
auto BlockManager::write_partial_block(block_id_t block_id, const u8 *data,
  ...
  {
    *(ptr + i) = * (data + i - offset);
//    std::cout << "read" << *(ptr2 + 5 * block_sz + block_sz - 1) << std::endl;
//    std::cout << "write " << (char) *(ptr + i) << "in block " << block_id << std::endl;
  }
  this->write_fail_cnt++;
  return KNullOk;
}
...
auto BlockManager::zero_block(block_id_t block_id) -> ChfsNullResult {
  ...
  u8 * ptr = this->block_data;
  ptr += block_id * block_sz;
  for (int i = 0; i < block_sz / sizeof(u8); i++)
  {
    *(ptr + i) = 0;
  }
  return KNullOk;
