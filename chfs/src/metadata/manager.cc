...
// { Your code here }
  auto InodeManager::allocate_inode(InodeType type, block_id_t bid, bool isdis)
  ...
  for (auto iter = iter_res.unwrap(); iter.has_next();
    ...
    if (free_idx) {
      ...
      auto inode = new Inode(type, bm->block_size(), isdis);
      std::vector<u8> buffer1(bm->block_size());
      inode->flush_to_buffer(buffer1.data());
      bm->write_block(bid, buffer1.data());
      inode_id_t res1 = count * bm->block_size() + free_idx.value();
      this->set_table(res1, bid);
      delete inode;
      return ChfsResult<inode_id_t>(RAW_2_LOGIC(res1));
...
auto InodeManager::set_table(inode_id_t idx, block_id_t bid) -> ChfsNullResult {
  ...
  bm->read_block(idx / (bm->block_size() / sizeof(block_id_t)) + 1, buffer2.data());
  auto temp =  (block_id_t *)buffer2.data();
  temp += idx % (bm->block_size() / sizeof(block_id_t));
  *temp = bid;
  bm->write_block(idx / (bm->block_size() / sizeof(block_id_t)) + 1, buffer2.data());
...
auto InodeManager::get(inode_id_t id) -> ChfsResult<block_id_t> {
  ...
  // table index.
  std::vector<u8> buffer2(bm->block_size());
  bm->read_block(LOGIC_2_RAW(id) / (bm->block_size() / sizeof(block_id_t)) + 1, buffer2.data());
  auto temp =  (inode_id_t *)buffer2.data();

  temp += LOGIC_2_RAW(id) % (bm->block_size() / sizeof(block_id_t));
  res_block_id = *temp;
...
auto InodeManager::free_inode(inode_id_t id) -> ChfsNullResult {
  ...
  std::vector<u8> buffer1(bm->block_size());
  bm->read_block(LOGIC_2_RAW(id) / (bm->block_size() / sizeof(block_id_t)) + 1, buffer1.data());
  auto temp = &(buffer1[(LOGIC_2_RAW(id) % (bm->block_size() / sizeof(block_id_t))) * ((sizeof(block_id_t)) / sizeof(u8))]);
  *((block_id_t *) temp) = 0;
  bm->write_block(LOGIC_2_RAW(id) / (bm->block_size() / sizeof(block_id_t)) + 1, buffer1.data());

  std::vector<u8> buffer2(bm->block_size());
  bm->read_block(LOGIC_2_RAW(id) / bm->block_size() + 1 + this->n_table_blocks, buffer2.data());
  auto bitmap = Bitmap(buffer2.data(), bm->block_size());
  bitmap.clear(LOGIC_2_RAW(id) % bm->block_size());
  bm->write_block(LOGIC_2_RAW(id) / bm->block_size() + 1 + this->n_table_blocks, buffer2.data());
