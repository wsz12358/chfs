...
public:
   ...
   */
  Inode(InodeType type, usize block_size, bool isdis=false)
      : type(type), inner_attr(), block_size(block_size) {
    CHFS_VERIFY(block_size > sizeof(Inode), "Block size too small");
    if (type == InodeType::FILE && isdis)
      nblocks = (block_size - sizeof(Inode)) / sizeof(BlockInfo);
    else if (type == InodeType::FILE && !isdis)
  ...
  auto set_blockinfo(usize idx, block_id_t bid, mac_id_t mid, version_t vid) { this->blockInfos[idx] = BlockInfo(bid, mid, vid); }
  ...
  auto get_type() -> InodeType {return type;}
