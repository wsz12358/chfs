...
BlockAllocator::BlockAllocator(std::shared_ptr<BlockManager> block_manager)
    : BlockAllocator(std::move(block_manager), usize(0), true) {}
...
BlockAllocator::BlockAllocator(std::shared_ptr<BlockManager> block_manager,
                               bool use_version, bool will_initialize)
          : bm(std::move(block_manager))
{
  if (!use_version) BlockAllocator(std::move(block_manager), usize(0), true);
  else
  {
        // calculate the total blocks required
    const auto total_bits_per_block = this->bm->block_size() * KBitsPerByte;
    version_block_cnt = this->bm->total_blocks() * sizeof(version_t) / this->bm->block_size();
    
    if (this->bm->total_blocks() % (this->bm->block_size() / sizeof(version_t)) != 0) {
      version_block_cnt += 1;
    }
    this->bitmap_block_id = version_block_cnt;
    ...
    // zeroing
    for (block_id_t i = 0; i < this->version_block_cnt; i++) {
      this->bm->zero_block(i);
    }
...
auto BlockAllocator::allocate() -> ChfsResult<block_id_t> {
  ...
  for (uint i = 0; i < this->bitmap_block_cnt; i++) {
    ...
    if (res) {
      ...
      retval = res.value() + i * KBitsPerByte * bm->block_size();
...
//    std::cout << "deallocate fail" << std::endl;
  ...
  std::vector<u8> buffer(bm->block_size());
  bm->read_block(block_id / (bm->block_size() * KBitsPerByte) + this->bitmap_block_id, buffer.data());
  auto bitmap = Bitmap(buffer.data(), bm->block_size());
  ...
  bm->write_block(block_id / (bm->block_size() * KBitsPerByte) + this->bitmap_block_id, buffer.data());

//    std::cout << "deallocate success" << std::endl;
  return KNullOk;
}
